...
def metrics_html(mae, avg_rel_err, accuracy_pct, has_gt):
    if not has_gt:
        return ('<div class="metrics-box">Chưa upload groundtruth.json nên không '
                'tính được Accuracy / MAE / Relative Error.</div>'
    ...
    <div class="metrics-box">
      ...
      <div>Accuracy (sai số ≤ {ACCURACY_TOLERANCE_PCT:.0f}%): <b>{accuracy_pct}%</b></div>
...
def run_demo(ref_image_file, video_file, devices_file, groundtruth_file,
    ...
    try:
        ...
        if within_tol:
            ...
            accuracy = round(100 * sum(within_tol) / len(within_tol), 1)
            m_html = metrics_html(mae, avg_rel, accuracy, True)
        else:
            m_html = metrics_html(None, None, None, False)

        cards = stat_cards_html(total, occupied, free, int(rate))
        return cards, table, m_html

    except Exception as e:
        err = f"Lỗi: {e}\n\n{traceback.format_exc()}"
        return empty_cards, [], f'<pre style="color:#dc2626;font-size:11px;white-space:pre-wrap;">{err}</pre>'
...
with gr.Blocks(title="Smart Gym Occupancy Management") as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            gr.Markdown('<div class="panel-title">📋 Setup Inputs</div>')
            ...
            run_btn = gr.Button("🔄 Re-Analyze", variant="primary")
